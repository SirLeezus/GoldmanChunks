...
public class ChunkListener implements Listener {
    ...
    private void warnMessage(Player player, boolean trusted, UUID owner, String type) {
        ...
        switch (type) {
            case "build":
                messageType = Lang.ITEM_SETTINGS_BUILD_NAME.getString(new String[] { plugin.getUtility().format("&cfalse") });
                break;
            case "break":
                messageType = Lang.ITEM_SETTINGS_BREAK_NAME.getString(new String[] { plugin.getUtility().format("&cfalse") });
                break;
            case "interact":
                messageType = Lang.ITEM_SETTINGS_INTERACT_NAME.getString(new String[] { plugin.getUtility().format("&cfalse") });
                break;
            case "pve":
                messageType = Lang.ITEM_SETTINGS_PVE_NAME.getString(new String[] { plugin.getUtility().format("&cfalse") });
                break;
            default:
                messageType = "ERROR";
        }
        ...
        if (plugin.getData().isPlayerAutoClaiming(e.getPlayer().getUniqueId())) {
            ...
            if (!SQL.isChunkClaimed(chunkCord)) {
                ...
                if (playerClaimAmount < playerMaxClaims) {
                    ...
                    plugin.getData().removePlayerAutoClaim(uuid);
                    player.sendMessage(Lang.PREFIX.getString(null) + Lang.ERROR_COMMAND_CLAIM_MAXED.getString(new String[] { plugin.getUtility().formatAmount(playerClaimAmount), plugin.getUtility().formatAmount(playerMaxClaims) }));
                }
